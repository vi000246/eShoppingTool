...
$(function(){
  $('html').attr('dir', 'ltr');
   ...
   $('.fx-ext-tab-trigger').click(function(e){
    ...
    /*** NOT YET
        var refElement = $("div.fees-panel:eq(1)");
        $(refElement).each(function() {
            $(refElement).position().top;
        });
        var refElement = $('.fees-panel:eq(1)').offset();

         $(window).on("scroll", function(){
            var endPoint = $('.fx-paper-card.fx-col-md-8:eq(1)').position().top + $('.fx-paper-card.fx-col-md-8:eq(1)').outerHeight();
            var scrollPos = $(this).scrollTop();
            if (scrollPos - 30 >= refElement.top){
                $(refElement).css({
                    'position':'fixed',
                    'top':'90px',
                    'bottom':'initial',
                ...
                if(scrollPos + $('.fx-paper-card.fx-col-md-8:eq(1)').outerHeight() - 90 >= endPoint){
                    $(refElement).css({
                        'position':'absolute',
                        'top':'initial',
                        'bottom':'15px',
